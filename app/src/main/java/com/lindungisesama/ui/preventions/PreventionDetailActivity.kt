package com.lindungisesama.ui.preventions
...
import com.lindungisesama.utils.PostContentHelper.convertTimestamp
...
import kotlinx.android.synthetic.main.activity_prevention_detail.*
import kotlinx.android.synthetic.main.layout_prevention_detail.*
import org.jetbrains.anko.AnkoLogger
import org.jetbrains.anko.error

class PreventionDetailActivity : AppCompatActivity(), AnkoLogger {

	companion object {
		const val PREVENTION_DETAIL_DATA = "prevention_detail_data"
	}

	private var isSourceEnabled = true
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		showLoading()
		try {
			val data: Result? = intent.getParcelableExtra(PREVENTION_DETAIL_DATA) as Result?
	...
	private fun setData(result: Result) {
		...
		result.first_publication_date?.let { timestamp ->
			text_timestamp.text = resources
				.getString(R.string.text_post_timestamp, convertTimestamp(timestamp))
		}
		result.data?.let { data ->
			...
			data.source.let { source ->
				if (source != null) {
					text_source.text = source.trim()
					button_source.setOnClickListener {
						FinestWebView.Builder(this).show(source)
					}
				} else isSourceEnabled = false
			}
												...
												RichSpanType.HYPERLINK -> j.start?.let {
													...
													setSpan(object : ClickableSpan() {
														override fun onClick(widget: View) {
															j.data?.url?.let { url ->
																FinestWebView
																	.Builder(this@PreventionDetailActivity)
																	.show(url)
															}
														}
									...
									override fun onResourceReady(
										...
										image.setOnClickListener {
											StfalconImageViewer.Builder(
												...
												Glide.with(this@PreventionDetailActivity)
													.load(image)
													.into(view)
	...
	private fun showLoading() {
		...
		button_source.visibility = View.GONE
