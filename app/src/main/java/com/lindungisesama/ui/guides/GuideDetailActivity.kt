...
import kotlinx.android.synthetic.main.layout_guide_detail.*
import org.jetbrains.anko.AnkoLogger
import org.jetbrains.anko.error

class GuideDetailActivity : AppCompatActivity(), AnkoLogger {

	companion object {
		const val GUIDE_DETAIL_DATA = "guide_detail_data"
	}
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		showLoading()
		try {
			val data: Result? = intent.getParcelableExtra(GUIDE_DETAIL_DATA) as Result?
												...
												RichSpanType.HYPERLINK -> j.start?.let {
													...
													setSpan(object : ClickableSpan() {
														override fun onClick(widget: View) {
															j.data?.url?.let { url ->
																FinestWebView
																	.Builder(this@GuideDetailActivity)
																	.show(url)
															}
														}
									...
									override fun onResourceReady(
										...
										image.setOnClickListener {
											StfalconImageViewer.Builder(
												...
												Glide.with(this@GuideDetailActivity)
													.load(image)
													.into(view)
